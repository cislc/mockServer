...
with open(r'i:\inspur_mock_server\mockServer\文档\科室编号', 'r', encoding='utf-8') as f:
...
# Doctor Differentiation Metadata
doc_specialties = {
...
sql_statements = [
    ...
    "SET NAMES utf8mb4;",
    "-- 清除旧的关联数据",
    "DELETE FROM doctor_schedule WHERE department_code IN ({});".format(','.join([f"'{v}'" for v in dept_mappings.values()])),
    "DELETE FROM doctor WHERE department_code IN ({});".format(','.join([f"'{v}'" for v in dept_mappings.values()])),
    "DELETE FROM department WHERE department_code IN ({});".format(','.join([f"'{v}'" for v in dept_mappings.values()])),
]
...
# 2. Generate Doctors & Schedules
sql_statements.append("\n-- 插入医生和排班数据")
for dept_name, dept_code in dept_mappings.items():
    specs = doc_specialties.get(dept_name, [f"{dept_name}常见病"])
    # Ensure at least 5 unique combinations or repeat
    for i in range(5):
        ...
        sql_doc = f"INSERT INTO doctor (doctor_code, doctor_name, department_code, title, specialty, doctor_des, doctor_image) VALUES ('{doc_code}', '{doc_name}', '{dept_code}', '{title_name}', '{specialty}', '{doc_des}', 'doc_{doc_code}.png');"
        ...
        for days_ahead in range(14):
            ...
            for session_name, session_code, start_t, end_t in daily_sessions:
                ...
                sql_sched = f"""INSERT INTO doctor_schedule (
                    ...
                    department_code, department_name, clinic_room_code, clinic_room_name, 
                    doctor_code, doctor_name, doctor_title_code, doctor_title, doctor_spec, 
                    doctor_sess_type_code, doctor_sess_type, service_code, service_name, 
                    ...
                    '{dept_code}', '{dept_name}', 'ROOM_{dept_code}_{random.randint(1,5)}', '{dept_name}门诊{random.randint(1,5)}诊室',
                    '{doc_code}', '{doc_name}', '{title_code}', '{title_name}', '{specialty}',
...
with open(r'i:\inspur_mock_server\mockServer\generate_mock.sql', 'w', encoding='utf-8') as f:
    f.write('\n'.join(sql_statements))
print("Mock SQL with differentiated doctor profiles generated.")
